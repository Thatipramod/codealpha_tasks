...
if STYLE_FILE.exists():
    st.markdown(
        f"<style>{STYLE_FILE.read_text()}</style>",
...
def build_regional_chart(df):
    ...
    ax.set_title(
        "Top 10 High-Unemployment Regions",
        fontsize=11,
        fontweight="bold",
    )
...
if df is None:
    ...
    st.info("Run analysis.py first to create the cleaned dataset.")
    st.stop()
...
st.sidebar.markdown(
    ...
    <div class="side-brand">
        ...
        <div>
            <div class="side-title">Unemployment</div>
            <div class="side-subtitle">Analytics Dashboard</div>
        </div>
    </div>
    """,
    unsafe_allow_html=True,
...
st.sidebar.markdown(
    ...
    <div class="developer-card">
        ...
        <div class="developer-role">CodeAlpha Data Science Internship</div>
    </div>
    """,
    unsafe_allow_html=True,
)
...
# Calculate date-wise overall rates
daily_rate = filtered_df.groupby("date")["unemployment_rate"].mean()

kpis = [
    ("Total Records", f"{len(filtered_df):,}"),
    ("Average Rate", f"{daily_rate.mean():.2f}%"),
    ("Peak Rate", f"{daily_rate.max():.2f}%"),
    ("Lowest Rate", f"{daily_rate.min():.2f}%"),
]
    ...
    if "covid_period" in filtered_df.columns:
        summary = (
            ...
            .rename(
                columns={
                    "covid_period": "Period",
                    "unemployment_rate": "Average Rate (%)",
                }
            )
        )
        ...
        st.caption("Data from March 2020 onward is categorized as the COVID-19 period.")


# Seasonal trends
with tabs[2]:
    st.pyplot(
        build_monthly_chart(filtered_df),
        use_container_width=True,
    )
...
# Footer
st.caption("Developed by Pramod | CodeAlpha Task 2 - Unemployment Analysis")
